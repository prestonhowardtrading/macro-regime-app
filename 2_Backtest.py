...
# Strip timezone from both indexes so comparison works
sp_daily.index = sp_daily.index.tz_localize(None) if sp_daily.index.tz else sp_daily.index
monthly.index  = monthly.index.tz_localize(None)  if monthly.index.tz  else monthly.index

# Resample S&P to month-end to match FRED monthly index
sp_monthly = sp_daily.resample("ME").last()

# Rebuild scores on the sp_monthly index so everything aligns
